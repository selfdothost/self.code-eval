...
def eval_script(path: Path):
    ...
    with tempfile.TemporaryDirectory() as outdir:
        ...
        if result.exit_code != 0:
            ...
            # javatuples must be on the *runtime* classpath too — an explicit
            # -cp overrides the CLASSPATH env, so include the jar here or every
            # program that returns an org.javatuples type dies with
            # NoClassDefFoundError at run time (compiled fine, fails to execute).
            result = run(
                ["java", "-ea", "-cp", f"{outdir}:{javatuples_path}", "Problem"],
                env=sys_env,
            )
            if result.timeout:
                status = "Timeout"
