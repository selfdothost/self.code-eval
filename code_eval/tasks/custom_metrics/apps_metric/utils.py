# Vendored verbatim from codeparrot/apps_metric (HF Hub Space)
# https://huggingface.co/spaces/codeparrot/apps_metric/blob/main/utils.py
# Pinned revision: 2a1d3d698c4e91ce7c6636000f6988be9abcac23 ("hide debugging
# comment", 2022-08-09) -- byte-identical to the metric's live HEAD as of
# 2026-07-18. See ../apps_metric/NOTICE for full provenance and why this is
# vendored.
#
# The only change from upstream is the `from .testing_util import run_test`
# relative import (unchanged in spirit -- this file already imported
# `run_test` from a sibling module upstream too, just via a package-relative
# import that assumed the HF Hub loader's dynamic module layout).
#
# `_compute()`/`compute_metrics()` has never accepted a `results_details`
# kwarg -- see NOTICE. code_eval/tasks/apps.py calls `evaluate_generations()`
# and `get_results()` directly (below) rather than going through
# `compute_metrics()`, so it can build per-problem/per-generation `details`
# from the real raw results shape.
...
from code_eval.tasks.custom_metrics.apps_metric.testing_util import run_test
